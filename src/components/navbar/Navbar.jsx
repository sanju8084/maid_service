...
const Navbar = () => {
  ...
  <ul className={`nav-links ${openMenu ? 'active' : ''}`}>
    {/* Mobile Profile shown first */}
    {isAuthenticated && loggedInUser && (
      <li className="user-profile mobile-only" ref={dropdownRef}>
    ...
    {isAuthenticated && loggedInUser ? (
      <li className="user-profile desktop-only" ref={dropdownRef}>
        <span className="hello-user">👋 Hello, {loggedInUser.name || 'User'}</span>
        <img
          src={loggedInUser.image || 'https://www.pngall.com/wp-content/uploads/5/Profile-Transparent.png'}
          alt="User"
      ...
      <li><NavLink onClick={() => setOpenMenu(false)} to="/login"><span className="loginbtn">Login</span></NavLink></li>
    )}
  </ul>
</nav>
